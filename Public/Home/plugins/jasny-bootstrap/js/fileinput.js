...
+function ($) { "use strict";
  ...
  var Fileinput = function (element, options) {
    ...
    // console.log(this.$element,this.$hidden,this.$input,this.$preview)
    this.original = {
  ...
  Fileinput.prototype.change = function(e) {
    ...
    if (files.length === 0) {
      ...
      if(/PersonalCenter/.test(location.href)){
        //账户页面不调用
      }else{
        var inputLength = $("div.fileinput").length;
        var exists = $("div.fileinput.fileinput-exists");
        var extLen = exists.length;
        var news = $("div.fileinput.fileinput-new");
        var newsLen = news.length;
        if( (newsLen < inputLength || newsLen == inputLength) && newsLen != 1){
          this.$element.insertAfter(news.eq(newsLen-1))
          this.$element.css({"display":"none"})
        }else{
          this.$element.insertAfter(exists.eq(extLen-1))
        }
      }
  ...
  Fileinput.prototype.clear = function(e) {
    ...
    this.$element.insertAfter($(".fileinput.fileinput-exists").eq(1))
  },
