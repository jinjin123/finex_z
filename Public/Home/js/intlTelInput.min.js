...
! function(a) {
	...
	a(b).on('load', function() {
		...
		_setInstanceCountryData: function() {
			...
			} else this.countries = k, this.countryCodes = l
		},
		...
		_generateMarkup: function() {
			...
			this.options.defaultStyling && (b += " " + this.options.defaultStyling), this.telInput.wrap(a("<div>", {
				"class": b
			}));
			var c = a("<div>", {
					"class": "flag-dropdown"
			...
			}).appendTo(c), this.preferredCountries.length && (this._appendListItems(this.preferredCountries, "preferred"), a("<li>", {
				"class": "divider"
			}).appendTo(this.countryList)), this._appendListItems(this.countries, ""), this.dropdownHeight = this.countryList.outerHeight(), this.countryList.removeClass("v-hide").addClass("hide"), this.countryListItems = this.countryList.children(".country")
		},
		...
		_initListeners: function() {
			...
			if(e.on("click" + this.ns, function() {
		...
		_handleLetterKey: function(b) {
			var c = String.fromCharCode(b),
				d = this.countryListItems.filter(function() {
		...
		_updateFlagFromInputVal: function() {
			var b = this,
				c = this._getDialCode(this.telInput.val());
			if(c) {
				var d = this.countryCodes[c.replace(/\D/g, "")],
					e = !1;
		...
		_resetToDialCode: function(a) {
			...
			// this.telInput.val(b);
			// 每次下拉选择后显示
			$('.selected-flag .flag').html('+' + a + '<i class="fa fa-angle-down"></i>');
			$('.telCode').val(a);
		},
		...
		_getCountryData: function(a, b) {
			for(var c = b ? k : this.countries, d = 0; d < c.length; d++)
				if(c[d].iso2 == a) return c[d];
			return null
		},
		...
		_updateNumber: function(b) {
			var c, d = this.telInput.val(),
				e = this._getDialCode(d);
			if(e.length > 1) c = d.replace(e, b), d == e && (c += this.options.dialCodeDelimiter);
			else {
		...
		isValidNumber: function(c) {
			var d = a.trim(this.telInput.val()),
				e = this.getSelectedCountryData(),
				f = c ? e.iso2 : "";
